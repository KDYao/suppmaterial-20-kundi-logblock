...
benchmark_settings_finer_local = {
    'HDFS': {
...
def read_log_characters(structured_log):
    ...
    try:
        ...
        #total_num_variables = len(all_variables)
    ...
    for chunkSize in chunkSizeList:
        ...
        for logpath in log_path_list:
            ...
            try:
                ...
                prep_time = cmd_execute_time(logzip.run)
                ...
                # Read characters
                dict_characters = read_log_characters(logzip.stuctured_log)
...
def cowic(dataset, setting, output_root_dir, compressor, chunkSizeList, repeat):
    ...
    # Change cowic config for each log type
    RunCowic.change_config(dataset)
    ...
    for chunkSize in chunkSizeList:
        ...
        for logpath in log_path_list:
            ...
            try:
                cowic_obj = RunCowic(logpath=logpath, out_dir=out_dir)

                # Copmress with cowic
                d_cw = cowic_obj.cowic_compress()

                # Compress with general compressor
                d_gc_all = get_compress_info(
                    path=logpath,
                    bucket_dir=out_dir,
                    compressor_list=compressor,
                    dataset=dataset,
                ...
                # d_gc_all return the result of same log block compressed by three different compressors separately
                for d_gc in d_gc_all:
                    d = {**d_cw, **d_gc}
...
def log_archive(dataset, setting, output_root_dir, compressor, chunkSizeList, repeat):
    ...
    for chunkSize in chunkSizeList:
        ...
        for logpath in log_path_list:
            ...
            try:
                log_archive_obj = RunLogArchive(logpath=logpath, out_dir=out_dir)

                # Copmress with cowic
                prep_time = cmd_execute_time(log_archive_obj.log_archive_compress)

                # Decompress bzip2 logs so we have the raw preprocessed files;
                # then we apply gzip/lz4/deflate on the tools

                subprocess.Popen('bzip2 -d *.bz2', shell=True, cwd=out_dir).wait()
...
def extra_bucket(dataset, setting, output_root_dir, compressor, chunkSizeList, repeat, template_dir='../template'):
    ...
    #FIXME
    original_log_template_path = os.path.join(template_dir, dataset + '.log.pkl')
    #original_log_template_path = os.path.join(template_dir, os.path.basename(setting['log_file']) + '.pkl')

    template_load_time_lst = []
    max_repeat = 10
    for i in range(0, max_repeat):
        ...
        # Template load time
        template_load_time_lst.append(template_load_time)
        del match_tree
    print('Loadting times: %s' % ','.join([str(round(x, 2)) for x in template_load_time_lst]))
    avg_time = sum(template_load_time_lst)/len(template_load_time_lst)
    print('Dataset: {dataset}, Average Loading time: {avg_time}'.format(dataset=dataset, avg_time=avg_time))
    return [{
        'Dataset': dataset,
        'AverageLoadingTime': avg_time,
        'Repeat': max_repeat
    }]
...
def compress_var_extraction_result(dirpath, compressor_settings, repeat=1, dict=None, type=1):
    ...
    # If non-content not in list, then it's splited separately
    # if len(_getAllFiles(folder=dirpath, ext='non-content.txt')) == 0:
    #     #iter_file_ext = ['.txt', '.csv']
    #     # Means all files
    #     iter_file_ext = ['']
    #     isOutputDetails = False

    filetype = ''
    #filetype_original_size = get_total_size_of_extension(dirpath, filetype)


    # If split by separate tokens, hashId.txt doesn't exist
    # if filetype_original_size == 0:
    #     continue
    ...
    #total_decompress_time = '{!s}'.format(sum(total_decompress_time))

    # if isOutputDetails:
    #
    #     dict['Prep: Original Size of %s' % filetype] = convert_size(filetype_original_size/repeat)
    #     dict['Prep: Compressed Size of %s' % filetype] = convert_size(filetype_compressed_size/repeat)
...
if __name__ == '__main__':
    ...
    result_out_path = '../result/%s_loading_result.csv' % opt.value
    ...
    for dataset, setting in benchmark_settings.items():
        ...
        if dataset in eval_datasets:
            ...
            def parallel_processing(
                ...
                output_data_dir = os.path.join(output_root_dir, opt.value)
                if not os.path.isdir(output_data_dir):
                    os.makedirs(output_data_dir)
                if opt == Option.S1:
                    # If based on transposing
                    dict = logblock(
                        dataset=dataset,
                        setting=setting,
