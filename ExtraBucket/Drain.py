"""
The code is from LogPai team, source code: https://github.com/logpai/logzip
...
class LogParser:
    def __init__(self, log_name, log_format, outdir='./result/', depth=4, st=0.4, maxChild=100, rex=[], keep_para=True):
        """
        Attributes
        ----------
    ...
    def parse(self, logfile):
        ...
        for idx, line in self.df_log.iterrows():
            ...
            if count % 1000 == 0 or count == len(self.df_log):
                print('[%s] ' % self.logName + 'Processed {0:.1f}% of log lines.'.format(count * 100.0 / len(self.df_log)))


        parsing_time = datetime.now() - start_time
        print('[%s] ' % self.logName + 'Parsing done. [Time taken: {!s}]'.format(parsing_time))
        return self.outputTemplate(logCluL), parsing_time
        #return self.outputResult(logCluL)

    def outputTemplate(self, logClustL):
        df_events = []
        idx = 0
        for logClust in logClustL:
            template_str = ' '.join(logClust.logTemplate)
            #template_id = hashlib.md5(template_str.encode('utf-8')).hexdigest()[0:8]
            template_id = "E" + str(idx)
            idx += 1
            df_events.append([template_id, template_str])
        return pd.DataFrame(data=df_events, columns=["EventId", "EventTemplate"])
